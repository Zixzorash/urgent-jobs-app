...
// --- Longdo Map Config ---
const LONGDO_KEY = '442b78f3234e68f33848b756aa3071f1';
...
const useLongdoMap = () => {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById(scriptId)) {
      ...
      style.href = 'https://api.longdo.com/map/css/map.css';
...
// 1. Longdo Map Picker (Hybrid: OSM Search -> Longdo Display)
const LongdoMapPicker = ({ lat, lng, onSelectLocation, placeholder }) => {
  ...
  useEffect(() => {
    if (isLoaded && !mapInstance.current && mapRef.current) {
      ...
      const initLon = lng || 100.5018;
      map.location({ lon: initLon, lat: initLat }, true);
      map.zoom(15);

      const marker = new longdo.Marker({ lon: initLon, lat: initLat }, { draggable: true });
      ...
      map.Event.bind('click', function() {
         ...
         if(mouseLoc && markerInstance.current) {
             markerInstance.current.move(mouseLoc);
             handleReverseGeocode(mouseLoc.lat, mouseLoc.lon);
         }
      });
  ...
  const handleReverseGeocode = async (lat, lon) => {
      try {
          ...
          if(onSelectLocation) onSelectLocation(lat, lon, name, data.display_name);
      } catch (e) {
          if(onSelectLocation) onSelectLocation(lat, lon, "ตำแหน่งที่เลือก", `Lat: ${lat.toFixed(5)}, Lon: ${lon.toFixed(5)}`);
      }
  };
  ...
  const handleSelectResult = (item) => {
      ...
      if(mapInstance.current) {
          ...
          if(markerInstance.current) markerInstance.current.move(loc);
          if(onSelectLocation) onSelectLocation(newLat, newLon, item.display_name.split(',')[0], item.display_name);
      }
  };

  if(!isLoaded) return <div className="h-48 bg-gray-100 flex items-center justify-center text-gray-400 rounded-xl">Loading Map...</div>;
    ...
    <div className="space-y-2">
      <div className="relative">
        <div className="flex items-center bg-gray-50 border border-gray-200 rounded-xl px-3 py-2">
           ...
           />
           {searchTerm && <button onClick={() => {setSearchTerm(''); setSearchResults([])}}><X className="w-4 h-4 text-gray-400"/></button>}
        </div>
        {searchResults.length > 0 && (
            <div className="absolute top-full left-0 right-0 bg-white border border-gray-100 shadow-lg rounded-b-xl z-20 max-h-40 overflow-y-auto">
                {searchResults.map((item, idx) => (
                    <div key={idx} onClick={() => handleSelectResult(item)} className="p-2 hover:bg-gray-50 cursor-pointer border-b border-gray-50 last:border-0 text-sm">
      ...
      <div ref={mapRef} className="rounded-xl overflow-hidden border border-gray-200 h-48 w-full bg-gray-100 relative z-0" />
      <div className="text-[10px] text-gray-400 text-right">Map by Longdo</div>
    </div>
  );
};

// 2. Longdo Map Viewer (Route/Display)
const LongdoMapViewer = ({ startLocation, endLocation, isRoute }) => {
  const mapRef = useRef(null);
  const mapInstance = useRef(null);
  const isLoaded = useLongdoMap();
  ...
  useEffect(() => {
      if(isLoaded && mapInstance.current) {
          ...
          if (isRoute && startLocation && endLocation) {
              ...
              map.Overlays.add(new longdo.Marker({ lat: endLocation.lat, lon: endLocation.lng }));
              ...
              setTimeout(() => {
                  map.bound(longdo.Util.locationBound([
  ...
  }, [isLoaded, startLocation, endLocation, isRoute]);

  if(!isLoaded) return <div className="h-48 bg-gray-100 flex items-center justify-center text-gray-400 rounded-xl">Loading Map...</div>;

  return <div ref={mapRef} className="w-full h-48 rounded-lg overflow-hidden border border-gray-200 mt-2 bg-gray-50 relative z-0" />;
};
        ...
        <header className={`${styles.primaryGradient} text-white px-5 py-4 flex items-center justify-between shadow-md z-20`}>
          ...
          <div className="flex items-center gap-3">
             ...
             <div onClick={() => navigateTo('profile')} className="w-9 h-9 rounded-full border-2 border-white/50 overflow-hidden cursor-pointer bg-white/20 flex items-center justify-center">
               {userData?.photoBase64 ? <img src={userData.photoBase64} className="w-full h-full object-cover" /> : <User className="w-5 h-5 text-white" />}
             </div>
          </div>
        </header>
      )}
